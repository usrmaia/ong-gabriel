...
describe("PatientFormAnamnesisSchema", () => {
  ...
  describe("PatientAttendanceSchema", () => {
    it("deve validar uma data dentro do intervalo permitido", () => {
      ...
      const result = PatientAttendanceSchema.safeParse({ dateAt: validDate });
    ...
    it("deve falhar se a data estiver vazia", () => {
      const result = PatientAttendanceSchema.safeParse({ dateAt: "" });
      expect(result.success).toBe(false);
    ...
    it("deve falhar se a data for inválida", () => {
      const result = PatientAttendanceSchema.safeParse({
    ...
    it("deve validar datas nos limites do intervalo permitido", () => {
      ...
      const resultMin = PatientAttendanceSchema.safeParse({
        dateAt: minDateStr,
      });
      const resultMax = PatientAttendanceSchema.safeParse({
        dateAt: maxDateStr,
      });
